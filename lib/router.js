...
function Router(opts) {
    ...
    Object.keys(opts).forEach(path => {
        ...
        let handler = opts[path].handler || opts[path];
        const meta = opts[path].handler ? opts[path] : {};

        if (typeof handler === 'string') {
            handler = require(handler);
        }

        this.use(path, handler, meta);
    });
}

Router.prototype.use = function(path, handler, store) {
    var self = this;
    ...
    function insertOperation(operation) {
        ...
        for (var i = 0, len = path.length; i < len; i++) {
            ...
            if (path.charCodeAt(i) === 58) {
                ...
                if (i === len) {
                    return self._insert(operation, path.slice(0, i), regex ? 3 : 1, params, handler, store, regex);
                }
                self._insert(operation, path.slice(0, i), regex ? 3 : 1, params, null, null, regex);

                // wildcard route
            } else if (path.charCodeAt(i) === 42) {
                self._insert(operation, path.slice(0, i), 0, null, null, null);
                params.push('*');
                return self._insert(operation, path.slice(0, len), 2, params, handler, store);
            }
        }
        // static route
        self._insert(operation, path, 0, params, handler, store);
    }
};

Router.prototype._insert = function(operation, path, kind, params, handler, store, regex) {
    var prefix = '';
    var pathLen = 0;
    var prefixLen = 0;
    var len = 0;
    ...
    while (true) {
        ...
        if (len < prefixLen) {
            ...
            if (len === pathLen) {
                ...
                currentNode.setHandler(operation, handler, params, store);
                currentNode.kind = kind;
            } else {
                // create a child node and add an handler to it
                node = new Node(path.slice(len), [], kind, null, regex);
                node.setHandler(operation, handler, params, store);
...
Router.prototype.lookup = function(context) {
    ...
    const metadata = this.find(context.operation,
